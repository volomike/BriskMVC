...
1. Install Bun from bun.com using their latest directions for that.

2. Ensure "bun" and "bunx" are in the environment path variables.

3. Create a new folder for the framework but do not put contents into it yet.

4. With your console, "cd" into this framework directory and type:
...
5. That will also have created a node_modules folder in there. That is necessary. Some of this system still requires NodeJS. Bun
does not yet have a production-grade MySQL driver as of 1/30/2026 but are getting closer. So, we use the NodeJS one.

6. Copy the contents of this repository into that folder.

7. Load the BriskMVC system with:

```
bun --hot server.js --watch controllers
```

8. Connection comes into your server.js and gets routed to the controllers based on your URL. So...
...
9. Likewise, unless you override the w.renderView(), the paths are mirrored but we use a "v-" prefix because it's easier when
...
10. The models are the serious coding of the system. The controllers should not do the heavy-lifting, just sequencing of events from
these models.

11. The server.js loads a w object which stands for "web". This provides many things like variables and functions for doing web
...
12. A central config file is avaiable in config/config.js. It's a JSON-ish file. It uses the hjson library to read it. So, you can
...
13. You don't need to reload the server service when you make changes usually if Bun is loaded with the --hot option. Just edit
...
14. If you have multiple projects that all share this same framework, then you may want to put the briskmvc folder in a central
...
15. I have created some useful Bash script to help you with this project:
...
16. I have used AI to provide an alternative briskmvc/router.js called briskmc/altrouter.js. You can switch your router to this by
